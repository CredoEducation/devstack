...
for arg in "$@"; do
    if [ $arg == "--studio" ]; then
        ...
        if [ ! "$(docker exec -t edx.devstack.ironwood.ecommerce bash -c 'echo "Course will be created for ecommerce"; exit $?')" ]; then
        ...
        if [ ! "$(docker exec -t edx.devstack.ironwood.marketing bash -c 'echo "Course will be created for marketing"; exit $?')" ]; then
...
if $ecommerce ; then
	...
	docker exec -t edx.devstack.ironwood.ecommerce bash -c "source /edx/app/ecommerce/ecommerce_env && python /edx/app/ecommerce/ecommerce/manage.py generate_courses '$course_json'"
fi

if $marketing ; then
	echo "Creating courses on marketing."
	docker exec -t edx.devstack.ironwood.marketing bash -c "drush generate_courses '$course_json'"
fi
